...
#define MPU6050_I2C_MST_CTRL        0x24u
...
esp_err_t mpu6050_config(mpu6050_handle_t sensor, const mpu6050_acce_fs_t acce_fs, const mpu6050_gyro_fs_t gyro_fs)
{
    ...
    uint8_t rate = 0x03;
    mpu6050_write(sensor, MPU6050_CONFIG, &rate, 1);

    // Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
    uint8_t smplrt_div = 0x04;
    mpu6050_write(sensor, MPU6050_SMPLRT_DIV, &smplrt_div, 1); // 200 Hz

    mpu6050_set_lpf(sensor, 100);
...
esp_err_t mpu6050_sample_rate(mpu6050_handle_t sensor, uint16_t rate)
{
    ...
    if ((rate < 4)) rate = 4;
    uint8_t sample_div[1] = { (1000 / rate - 1) };
    mpu6050_write(sensor, MPU6050_SMPLRT_DIV, sample_div, sizeof(sample_div));
    return mpu6050_set_lpf(sensor, rate / 2);
}
esp_err_t mpu6050_set_lpf(mpu6050_handle_t sensor, uint16_t lpf)
{
    ...
    return mpu6050_write(sensor, MPU6050_CONFIG, &data, 1);
}

esp_err_t mpu6050_calibrate(mpu6050_handle_t sensor) {
    mpu6050_write(sensor, MPU6050_PWR_MGMT_1, (uint8_t*) { 0x80 }, 1);
    delay(100);
    mpu6050_write(sensor, MPU6050_PWR_MGMT_1, (uint8_t*) { 0x01 }, 1);
    mpu6050_write(sensor, MPU6050_PWR_MGMT_2, (uint8_t*) { 0x00 }, 1);
    delay(200);

    // Configure device for bias calculation
    mpu6050_write(sensor, MPU6050_INTR_ENABLE, (uint8_t*) { 0x00 }, 1);   // Disable all interrupts
    mpu6050_write(sensor, MPU6050_FIFO_EN, (uint8_t*) { 0x00 }, 1);      // Disable FIFO
    mpu6050_write(sensor, MPU6050_PWR_MGMT_1, (uint8_t*) { 0x00 }, 1);   // Turn on internal clock source
    mpu6050_write(sensor, MPU6050_I2C_MST_CTRL, (uint8_t*) { 0x00 }, 1); // Disable I2C master
    mpu6050_write(sensor, MPU6050_USER_CTRL, (uint8_t*) { 0x00 }, 1);    // Disable FIFO and I2C master modes
    mpu6050_write(sensor, MPU6050_USER_CTRL, (uint8_t*) { 0x0C }, 1);    // Reset FIFO and DMP
    delay(15);
