...
#define TAG "BETA FLIGHT"

volatile float twoKp = (8.5f);	// 2 * proportional gain 比例增益
volatile float twoKi = (0.008f);	// 2 * integral gain     积分增益
volatile float sampleFreq = 40.0f;	    // sample frequency in Hz
...
static void quaternion2Angle() {
    ...
    ESP_LOGI(TAG, "angle yaw:%.3f, pitch:%.3f, roll:%.3f\n", yaw, pitch, roll);
}


// 默认取样个数
#define DEFAULT_CALIBRATION_NUMSAMPLES 500
// 误差抖动 阈值
#define DEFAULT_CALIBRATION_ACCEL_DEADZONE  0.01*0.01
#define DEFAULT_CALIBRATION_GYRO_DEADZONE  0.1*0.1
...
int imu_calibrate()
{
    ...
    for (int i = 0;i < 10;i++)
    {
        ...
        if (acce.acce_x * acce.acce_x > DEFAULT_CALIBRATION_ACCEL_DEADZONE
            || acce.acce_y * acce.acce_y > DEFAULT_CALIBRATION_ACCEL_DEADZONE
            || (acce.acce_z - 1) * (acce.acce_z - 1) > DEFAULT_CALIBRATION_ACCEL_DEADZONE
...
static void imuTask(void* param) {
    ...
    for (;;)
    {
        ...
        // sampleFreq = (float)(1000.0f / (now - last_update));
        // last_update = now;

        // update mahony imu
        MahonyAHRSupdateIMU(gyro.gyro_x * 0.0174533f, gyro.gyro_y * 0.0174533f, gyro.gyro_z * 0.0174533f, acce.acce_x, acce.acce_y, acce.acce_z);
