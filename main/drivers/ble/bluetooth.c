...
#define GATTS_DESCR_UUID_BATTERY     0x4444
#define GATTS_NUM_HANDLE_BATTERY     4

// profile a
#define GATTS_SERVICE_UUID_NMEA   0x00FF
#define GATTS_CHAR_UUID_NMEA      0xFF01
#define GATTS_DESCR_UUID_NMEA     0x3333
#define GATTS_NUM_HANDLE_NMEA     4

#define GATTS_SERVICE_UUID_RTCM   0x00EE
#define GATTS_CHAR_UUID_RTCM      0xEE01
#define GATTS_DESCR_UUID_RTCM     0x2222
#define GATTS_NUM_HANDLE_RTCM     4
...
static esp_gatt_if_t gatt_if_one = 0xff;
static esp_gatt_if_t gatt_if_two = 0xff;
static uint16_t gnss_conn_one = 0xffff;
static uint16_t gnss_conn_two = 0xffff;

#define MAX_SIZE 500

Queue* queue = NULL;
QueueHandle_t Qhandle; // 线程安全
void insert_queue(QueueHandle_t handle, const uint8_t* data, uint32_t len)
{
    uint32_t i = 0;
    while (i < len)
        if (xQueueSend(handle, data + (i++), 0) != pdPASS) {
            ESP_LOGI(TAG, "rtcm 发送失败");
            return;
        }
}
static uint16_t mtu = 500;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t* param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        gl_profile_tab[PROFILE_A_APP_ID].service_id.id.uuid.uuid.uuid16 = GATTS_SERVICE_UUID_NMEA;
...
#endif
        // 创建服务
        esp_ble_gatts_create_service(gatts_if, &gl_profile_tab[PROFILE_A_APP_ID].service_id, GATTS_NUM_HANDLE_NMEA);
        break;
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        gl_profile_tab[PROFILE_A_APP_ID].char_uuid.uuid.uuid16 = GATTS_CHAR_UUID_NMEA;
        // 开启服务
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        gnss_conn_one = param->connect.conn_id;
        gatt_if_one = gatts_if;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t* param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        gl_profile_tab[PROFILE_B_APP_ID].service_id.id.uuid.uuid.uuid16 = GATTS_SERVICE_UUID_RTCM;

        esp_ble_gatts_create_service(gatts_if, &gl_profile_tab[PROFILE_B_APP_ID].service_id, GATTS_NUM_HANDLE_RTCM);
        break;
    case ESP_GATTS_READ_EVT: {
        // ESP_LOGI(TAG, "GATT_READ_EVT, conn_id %d, trans_id %d, handle %d\n", param->read.conn_id, param->read.trans_id, param->read.handle);
        esp_gatt_rsp_t rsp;
    ...
    case ESP_GATTS_WRITE_EVT: {
        // ESP_LOGI(TAG, "GATT_WRITE_EVT, conn_id %u, trans_id %u, handle %u\n", param->write.conn_id, param->write.trans_id, param->write.handle);
        if (!param->write.is_prep) {
            ESP_LOGI(TAG, "GATT_WRITE_EVT, value len %d, value :", param->write.len);
            esp_log_buffer_hex(TAG, param->write.value, param->write.len);


            insert_queue(Qhandle, param->write.value, param->write.len);
        ...
        gatts_write_event_env(gatts_if, &b_prepare_write_env, param);
        break;
    }
    case ESP_GATTS_EXEC_WRITE_EVT:
        ESP_LOGI(TAG, "ESP_GATTS_EXEC_WRITE_EVT");
        esp_ble_gatts_send_response(gatts_if, param->write.conn_id, param->write.trans_id, ESP_GATT_OK, NULL);
        gatts_exec_write_event_env(&b_prepare_write_env, param);
        break;
    case ESP_GATTS_MTU_EVT:
        ESP_LOGI(TAG, "ESP_GATTS_MTU_EVT, MTU %d", param->mtu.mtu);
        break;
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        gl_profile_tab[PROFILE_B_APP_ID].char_uuid.uuid.uuid16 = GATTS_CHAR_UUID_RTCM;
    ...
    case ESP_GATTS_CONNECT_EVT:
        ...
        gnss_conn_two = param->connect.conn_id;
        gatt_if_two = gatts_if;
        break;
    case ESP_GATTS_CONF_EVT:
        ESP_LOGI(TAG, "ESP_GATTS_CONF_EVT status %d attr_handle %d", param->conf.status, param->conf.handle);
        if (param->conf.status != ESP_GATT_OK) {
...
static void gatts_profile_c_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t* param) {
    ...
    case ESP_GATTS_REG_EVT:
        ...
        gl_profile_tab[PROFILE_C_APP_ID].service_id.id.uuid.uuid.uuid16 = GATTS_SERVICE_UUID_BATTERY;

        esp_ble_gatts_create_service(gatts_if, &gl_profile_tab[PROFILE_C_APP_ID].service_id, GATTS_NUM_HANDLE_BATTERY);
        break;
    case ESP_GATTS_CREATE_EVT:
        ESP_LOGI(TAG, "CREATE_SERVICE_EVT, status %d,  service_handle %d\n", param->create.status, param->create.service_handle);
        ...
        gl_profile_tab[PROFILE_C_APP_ID].char_uuid.uuid.uuid16 = GATTS_CHAR_UUID_BATTERY;
...
void sendByBt(const char* data, uint32_t len) {
    ...
    while (total < len) {
        length = len - total;
        if (mtu < length) length = mtu;

        esp_ble_gatts_send_indicate(gatt_if_one, gnss_conn_one, gl_profile_tab[PROFILE_A_APP_ID].char_handle,
            length, data + total, false);
